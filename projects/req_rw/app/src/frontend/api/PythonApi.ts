...
import { appSetPath, appClearError, appLoadFileState } from '../store/appSlice';
...
export class PythonApi extends BaseApi {
  ...
  override async init(): Promise<ApiResult> {
    ...
    if (state.lastFilepath) {
      const result = await this._load(state.lastFilepath);
      if (!result.ok) {
        this.dispatch(appClearError());
      }
      return { ok: true };
    }
  ...
  private async _load(filepath: string): Promise<ApiResult> {
    ...
    try {
      ...
      const prefsResult = await this.getPrefs();
      if (prefsResult.ok && prefsResult.data?.file_state) {
        const entry = prefsResult.data.file_state[filepath];
        if (entry) {
          this.dispatch(appLoadFileState({ viewName: entry.active_view ?? null, views: entry.views ?? {} }));
        }
      }
      return { ok: true };
    } catch (err) {
  ...
  override async getPrefs(): Promise<ApiResult<Prefs>> {
    const res = await fetch(`${this.baseUrl}/prefs`);
    if (!res.ok) return { ok: false, error: `Failed to get prefs: ${res.statusText}` };
  ...
  override async saveFileState(key: string, state: FileViewState): Promise<ApiResult> {
    const res = await fetch(`${this.baseUrl}/prefs`, {
      ...
      body: JSON.stringify({ file_state: { [key]: state } }),
    });
    if (!res.ok) return { ok: false, error: `Failed to save prefs: ${res.statusText}` };
    return { ok: true };
  }
