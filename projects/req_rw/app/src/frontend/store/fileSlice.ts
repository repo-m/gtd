...
import { FileState, Req, FieldDef, getNewFileState } from './file';
...
function updateMeta(state: FileSliceState): void {
  ...
  if (state.root !== null) traverse(state.root, 1, '0');
}
...
function addState(
  ...
  if (importState.root === null) return;
  ...
  reducers: {
    ...
    fileCreateNextReq: fileHistoryAdapter.undoableReducer(
      (state, action: PayloadAction<number | undefined>) => {
        ...
        if (afterId === undefined) {
          if (state.root !== null) state.requirements[state.root].children.push(newId);
        } else {
    ...
    fileUpdate: fileHistoryAdapter.undoableReducer(
      (
        state,
        action: PayloadAction<
          Partial<Pick<FileState, 'description' | 'fields' | 'types'>>
        >,
      ) => {
    ...
    fileSave(_state) {
      // Signal-only: appSlice clears isDirty on this action
    },
    fileSaveAs(_state) {
      // Signal-only: appSlice clears isDirty on this action
    },
