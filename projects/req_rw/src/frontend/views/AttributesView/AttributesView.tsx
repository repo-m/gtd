...
export function AttributesView() {
  ...
  const { title, prefix, description, fields, views } = useSelector(
  ...
  return (
    <Modal title="Edit Attributes">
      ...
      >
        <button onClick={handleCancel} style={footerBtnStyle}>
          CANCEL
        </button>
        <button onClick={handleOK} style={{ ...footerBtnStyle, background: 'var(--color-accent)', color: 'var(--color-text-inverse)', borderColor: 'var(--color-accent)' }}>
          OK
        </button>
      </div>
    </Modal>
